...
const percentages = [80, 95, 90, 75, 75, 70, 70, 70];
...
const mainFn = () => {
  ...
  if (
    window.pageYOffset >= section2Offset - 100 &&
    window.pageYOffset < section3Offset - 100
  ) {
    percentBars.forEach((percentBar, i) => {
      percentBar.style.width = `${percentages[i]}%`;
      percentBar.previousElementSibling.firstElementChild.textContent =
        percentages[i];
    });
  } else {
    percentBars.forEach((percentBar, i) => {
